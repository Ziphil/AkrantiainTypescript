...
export abstract class Matchable {
   ...
   * マッチしなかった場合は `-1` を返します。*/
  public abstract matchRight(stat: Stat, from: number, module: Module): number;

  /** ちょうど `to` で与えられた位置から左向きにマッチするかどうかを調べます。
   * マッチした場合はマッチした範囲の左端のインデックス (範囲にそのインデックス自体を含む) を返します。
   * マッチしなかった場合は `-1` を返します。*/
  public abstract matchLeft(stat: Stat, to: number, module: Module): number;
  ...
  public abstract isConcrete(): boolean;

  /** モジュール内に存在しない識別子を含んでいればそれを返し、そうでなければ undefined を返します。*/
  public abstract findUnknownIdentifier(module: Module): Identifier | undefined;

  /** 識別子定義文を全て展開したときに `identifiers` に含まれる識別子が含まれていればそれを返し、そうでなければ `undefined` を返します。
   * 識別子の定義が循環参照していないかを調べるのに用いられます。*/
  public abstract findCircularIdentifier(identifiers: Array<Identifier>, module: Module): Identifier | undefined;
